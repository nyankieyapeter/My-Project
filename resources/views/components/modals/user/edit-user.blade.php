...
<div class="modal fade" id="editUser{{ $user->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
     ...
     <form action="{{ route('user.update', ['store' => session('selected_store'), 'user' => $user->id]) }}"
           method="POST">
         @csrf
         @method('PATCH')
                 ...
                 <div class="form-group">
                     <label class="fw-bold" for="user_email" style="font-size: 12px">E-Mail</label>
                      <input required
                         ...
                         type="email" class="form-control @error('user_email') is-invalid @enderror"E-Ma
                         style="font-size: 12px">
                 </div>
             </div>
             <div class="col-md-12 mt-2">
                 ...
                 <div class="form-group">
                    ...
                    <select name="user_role" class="form-control" style="font-size: 12px">
                        ...
                        {{--@foreach(App\Models\Store::all() as $store)
                            <option value="{{ $store->id }}" {{ $supplier->store->id == $store->id ? 'selected' : '' }}>
                                {{ $store->store_name }}
                            </option>
                        @endforeach--}}
                    </select>
